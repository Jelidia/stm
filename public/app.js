...
function renderFavs() {
  ...
  for (const f of favs) {
    ...
    li.querySelector('[data-act="del"]').onclick = () => {
      saveFavs(favs.filter(x => x.id !== f.id));
    };
...
addForm.onsubmit = async (e) => {
  ...
  // Resolve once to ensure it's a real stop
  const m = await fetch(`/api/resolve?q=${encodeURIComponent(query)}`).then(r => r.json());
  if (!m.length) { alert("Stop not found. Try a stop code like 50410."); return; }
...
async function viewFav(f) {
  ...
  const data = await fetch(`/api/stop/${encodeURIComponent(f.query)}?max=5`).then(r => r.json());
  if (data.error) {
  ...
  if (!items.length) {
    results.innerHTML = `<div class="card">No realtime arrivals found right now.</div>`;
    return;
  }
  results.innerHTML = "";
  for (const a of items) {
    ...
    div.innerHTML = `
      ...
      ${v ? `<div><b>Vehicle:</b> ${v.lat.toFixed(6)}, ${v.lon.toFixed(6)} (${v.distance_m_to_stop} m away${v.bearing!=null?`, ${Math.round(v.bearing)}°`:''})  —  <a href="${gmaps}" target="_blank">Open map</a></div>` : `<div><b>Vehicle:</b> unknown</div>`}
    `;
